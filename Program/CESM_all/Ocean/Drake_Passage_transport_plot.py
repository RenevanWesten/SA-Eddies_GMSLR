#Program plots the Drake Passage volume transport
...
time, transport_cesm		= ReadinData(directory_cesm+'/Ocean/Drake_Passage_transport_depth_'+str(depth_min)+'-'+str(depth_max)+'_m.nc')
time_year, transport_cesm_year	= YearlyConverter(time, transport_cesm, month_start, month_end)
 
#-----------------------------------------------------------------------------------------

time, transport_cesm_control	                = ReadinData(directory_cesm_control+'/Ocean/Drake_Passage_transport_depth_'+str(depth_min)+'-'+str(depth_max)+'_m.nc')
...
time, transport_cesm_low		= ReadinData(directory_cesm_low+'/Ocean/Drake_Passage_transport_depth_'+str(depth_min)+'-'+str(depth_max)+'_m.nc')
time_year, transport_cesm_low_year	= YearlyConverter(time, transport_cesm_low, month_start, month_end)

#-----------------------------------------------------------------------------------------

time, transport_cesm_low_control	            = ReadinData(directory_cesm_low_control+'/Ocean/Drake_Passage_transport_depth_'+str(depth_min)+'-'+str(depth_max)+'_m.nc')
...
CESM_graph	= ax.plot_date(time_year, transport_cesm_control_year, '-k', linewidth = 2.0, label = 'HR-CESM Control')
Control_graph	= ax.plot_date(time_year, transport_cesm_year, '-r', linewidth = 2.0, label = 'HR-CESM')
...
graphs	      = CESM_graph + Control_graph

legend_labels = [l.get_label() for l in graphs]
ax.legend(graphs, legend_labels, loc='lower left',
 		  ncol=1, fancybox=True, shadow=False, numpoints = 1)

ax.set_title('HR-CESM')

#-----------------------------------------------------------------------------------------
fig, ax	= subplots()

CESM_low_graph	    = ax.plot_date(time_year, transport_cesm_low_control_year, '-k', linewidth = 2.0, label = 'LR-CESM Control')
Control_low_graph  = ax.plot_date(time_year, transport_cesm_low_year, '-r', linewidth = 2.0, label = 'LR-CESM')
...
graphs	      = CESM_low_graph + Control_low_graph
