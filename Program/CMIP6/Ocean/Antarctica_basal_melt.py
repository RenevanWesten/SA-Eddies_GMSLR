...
def TrendRemoverControl(time, control_data, data):
	"""Removes drift in Control time series"""
...
#Get the model names and path
models_LARMIP = glob.glob(directory+'../LARMIP/RFunctions/RF*R1.dat')
models_LARMIP.sort()

for model_i in range(len(models_LARMIP)):
	#Only retain the model names
	index_start		= models_LARMIP[model_i].find('RF_') + 3
	index_end		= models_LARMIP[model_i].find('_BM08')
	models_LARMIP[model_i]	= models_LARMIP[model_i][index_start:index_end]
...
for model_i in range(len(models)):
	...
	filename	= directory+models[model_i]+'/Ocean/TEMP_Southern_Ocean_Regions.nc'
	filename_control= directory+models[model_i]+'/Ocean/TEMP_Southern_Ocean_Regions_Control.nc'
	...
	SLR_1_all	= ma.masked_all((len(models_LARMIP), len(per_levels), len(time_year)))
	SLR_2_all	= ma.masked_all((len(models_LARMIP), len(per_levels), len(time_year)))
	SLR_3_all	= ma.masked_all((len(models_LARMIP), len(per_levels), len(time_year)))
	SLR_4_all	= ma.masked_all((len(models_LARMIP), len(per_levels), len(time_year)))
	SLR_5_all	= ma.masked_all((len(models_LARMIP), len(per_levels), len(time_year)))
	SLR_total_all	= ma.masked_all((len(models_LARMIP), len(per_levels), len(time_year)))

	#-----------------------------------------------------------------------------------------

	for model_j in range(len(models_LARMIP)):
		#Loop over each Ice Sheet model
		print model_j, len(models_LARMIP)

		for region_i in range(1, 6):
			#Loop over each region
			filename	= directory+'../LARMIP/RFunctions/RF_'+models_LARMIP[model_j]+'_BM08_R'+str(region_i)+'.dat'
		...
		SLR_1_all[model_j]	= SLR_1[per_index]
		SLR_2_all[model_j]	= SLR_2[per_index]
		SLR_3_all[model_j]	= SLR_3[per_index]
		SLR_4_all[model_j]	= SLR_4[per_index]
		SLR_5_all[model_j]	= SLR_5[per_index]
		SLR_total_all[model_j]	= SLR_total[per_index]
        
	TEMP_data = netcdf.Dataset(directory+models[model_i]+'/Ocean/Antarctica_basal_melt_SLR.nc', 'w')
	TEMP_data.createDimension('PER_LEVELS', len(per_levels))
	TEMP_data.createDimension('MODELS', len(models_LARMIP))
	...
	TEMP_data.variables['MODELS'][:] 		= np.arange(len(models_LARMIP)) + 1
